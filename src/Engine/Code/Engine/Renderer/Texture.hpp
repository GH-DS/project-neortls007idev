...
public:
	 ...
	 Texture( RenderContext* renderContext , ID3D12Texture2D* handle );
	 Texture( const char* imageFilePath, unsigned int textureID, IntVec2 dimensions );
	 Texture( const char* filePath, RenderContext* renderContext , ID3D12Texture2D* handle );
	 //Texture( Rgba8 color , RenderContext* renderContext , ID3D12Texture2D* handle );
	~Texture();
	...
	const    IntVec2 GetDimensions() const;
			 ID3D12Texture2D* GetHandle()																				{ return m_handle; }
...
private:
	...
	ID3D12Texture2D*	m_handle				= nullptr;
