...
Texture::Texture( RenderContext* renderContext , ID3D12Texture2D* handle ) :
...
Texture::Texture( const char* filePath , RenderContext* renderContext , ID3D12Texture2D* handle ) : Texture( renderContext , handle )
{
...
// 
// Texture::Texture( Rgba8 color , RenderContext* renderContext , ID3D12Texture2D* handle ) : Texture( renderContext , handle )
// {
// 	m_color = color;
// }
...
TextureView* Texture::GetOrCreateShaderResourceView()
{
	...
	ID3D12Device* device = m_owner->m_device;
	ID3D12ShaderResourceView* srv = nullptr;
...
TextureView* Texture::GetOrCreateShaderAndDepthStencilResourceView( Vec2 dimension )
{
	...
	ID3D12Device* device = m_owner->m_device;
	ID3D12DepthStencilView* dsv = nullptr;

	D3D12_TEXTURE2D_DESC descDepth;
	...
	descDepth.Usage = D3D12_USAGE_DEFAULT;
	descDepth.BindFlags = D3D12_BIND_DEPTH_STENCIL | D3D12_BIND_SHADER_RESOURCE;
	descDepth.CPUAccessFlags = 0;
	descDepth.MiscFlags = 0;
	device->CreateTexture2D( &descDepth , NULL , &m_handle );

	D3D12_DEPTH_STENCIL_VIEW_DESC descDSV;
	memset( &descDSV , 0 , sizeof( D3D12_DEPTH_STENCIL_VIEW_DESC ) );
...
TextureView* Texture::GetOrCreateUnorderedAccessView()
{
	...
	ID3D12Device* device = m_owner->m_device;
	ID3D12UnorderedAccessView* uav = nullptr;
...
TextureView* Texture::GetOrCreateCubeMapShaderResourceView()
{
	...
	CD3D12_SHADER_RESOURCE_VIEW_DESC viewDesc;
		//memset( &view_desc , 0 , sizeof( CD3D12_SHADER_RESOURCE_VIEW_DESC ) );
